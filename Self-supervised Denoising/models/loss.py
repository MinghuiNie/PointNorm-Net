...
class UnsupervisedLoss(nn.Module):
    ...
    def stochastic_neighborhood(self, inputs):
        ...
        return knn_idx, mask, prob, knn_dist
          
    def forward(self, preds, inputs,  guess, score, epoch, **kwargs):
    #def forward(self, preds, inputs, epoch, **kwargs):
        ...
        input_nbh_idx, input_nbh_mask, _, input_nbh_dist= self.stochastic_neighborhood(inputs)    # (B, N, k), (B, N, k)
        sigma = input_nbh_dist[:,:,torch.arange(0,12)].mean(2)

        #Random operation
        input_nbh_pos_averagre = torch.zeros(8,1024,200, 3, dtype=torch.float).cuda()
        for i in range(200):  
            id = random.sample(range(0,64),4)
            input_nbh_idx_sel = input_nbh_idx[:,:,id]
            input_nbh_pos = group(inputs, idx=input_nbh_idx_sel)
            input_nbh_pos_averagre[:,:,i,:] = torch.mean(input_nbh_pos, dim=2)
        #Selection
        input_nbh_points = group(inputs, idx=input_nbh_idx)
        nbh_point_dist = torch.zeros(8,1024,200, 64, dtype=torch.float).cuda()
        for i in range(8):
            nbh_point_dist[i, :, :, :] = torch.bmm(input_nbh_pos_averagre[i,:,:,:], input_nbh_points[i,:,:,:].transpose(2, 1))**2
        nbh_dis = torch.sum(torch.exp(-nbh_point_dist/sigma.unsqueeze(2).unsqueeze(3)), 3)
        [v, id] = torch.sort(nbh_dis,2)
        id_sel = id[:,:,torch.arange(20,200)]
        input_nbh_pos_averagre_sel = torch.zeros(8,1024,180, 3, dtype=torch.float).cuda()
        for i in range(8):
            input_nbh_pos_averagre_sel[i, :, :, :] = gather(input_nbh_pos_averagre[i,:,:,:],id_sel[i,:,:])#(8,1024,180,3)
        #Loss optimization
        dist = (permuted_preds.unsqueeze(dim=-2).expand_as(input_nbh_pos_averagre_sel) - input_nbh_pos_averagre_sel)  # (B, N, k, 3)
        dist = -torch.exp(-torch.norm(dist, p=2, dim=3)**2/sigma.unsqueeze(2)).mean(-1)
        ...
        ##avg_dist = dist.sum(dim=-1) / 200    # (B, N), average distance
        avg_dist = dist
