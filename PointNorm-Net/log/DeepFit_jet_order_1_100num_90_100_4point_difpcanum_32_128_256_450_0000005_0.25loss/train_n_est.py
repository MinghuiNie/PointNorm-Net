...
def parse_arguments():
    ...
    parser.add_argument('--name', type=str, default='DeepFit_jet_order_1_100num_90_100_4point_difpcanum_32_128_256_450_0000005_0.25loss', help='training run name')
    parser.add_argument('--arch', type=str, default='simple', help='arcitecture name:  "simple" | "3dmfv"')
    parser.add_argument('--desc', type=str, default='if< 0.02 knn=18, if< 0.14 knn = 64, if< 0.16 knn = 128, else knn = 450; select 50', help='description')
    parser.add_argument('--indir', type=str, default='/home/zhangjie/桌面/unsupervised-normal-estimation/data/pclouds/', help='input folder (point clouds)')
    ...
    parser.add_argument('--saveinterval', type=int, default='20', help='save model each n epochs')
    parser.add_argument('--refine', type=int, default=True,  help='flag to refine the model, path determined by outri and model name')
    parser.add_argument('--refine_epoch', type=int, default=770, help='refine model from this epoch')
    ...
    parser.add_argument('--points_per_patch', type=int, default=200, help='max. number of points per patch')
...
def train_pcpnet(opt):
    ...
    os.system('cp dataset.py %s' % (log_dirname))  # backup the current dataset file
    ...
    for epoch in range(opt.nepoch):
        ...
        for train_batchind, data in train_enum:
            ...
            loss_all, loss, n_loss, _, consistency_loss, normal_loss = compute_loss(
                pred=pred, target=target, A_matrix = A_matrix,
                outputs=opt.outputs,
            ...
            while test_fraction_done <= train_fraction_done and test_batchind+1 < test_num_batch:
                ...
                loss_all, loss, n_loss, err_angle, consistency_loss, normal_loss = compute_loss(
                    pred=pred, target=target, A_matrix = A_matrix, 
                    outputs=opt.outputs,
                    output_pred_ind=output_pred_ind,
                    output_target_ind=output_target_ind,
...
def compute_loss(pred, target, A_matrix, outputs, output_pred_ind, output_target_ind, output_loss_weight, normal_loss_type, arch,
        ...
        if o == 'unoriented_normals' or o == 'oriented_normals':
            ...
            if o == 'unoriented_normals':
                if normal_loss_type == 'ms_euclidean':
                    ...
                    if o_target.shape==o_pred.shape:
                        normal_loss = 0.5 * torch.norm(torch.cross(o_pred, o_target, dim=-1), p=2, dim=1).mean()
                    else:
                        o_pred = o_pred.unsqueeze(1)
                        #normal_loss = 0.5 * torch.norm(torch.cross(o_pred.expand(o_target.size()), o_target, dim=-1), p=2, dim=2).mean()
                        normal_loss = -0.5 * torch.exp(-torch.norm(torch.cross(o_pred.expand(o_target.size()), o_target, dim=-1), p=2, dim=2)**2/0.25).mean()
                else:
            ...
            if use_consistency:
                ...
                A_loss = 0.000005*torch.bmm(torch.bmm(point_weights.unsqueeze(1),A_matrix), point_weights.unsqueeze(2)).mean()
                regularizer = regularizer_trans + regularizer
                #consistency_loss = consistency_loss + regularizer
                consistency_loss = regularizer

                loss = consistency_loss + normal_loss
                loss_all = loss + A_loss
        else:
            raise ValueError('Unsupported output type: %s' % (o))

    return loss_all, loss, n_loss, err_angle, consistency_loss, normal_loss
