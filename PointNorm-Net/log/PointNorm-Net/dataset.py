...
def load_shape(point_filename, weights_filename, features_filename, normals_filename, curv_filename, pidx_filename):
    pts = np.load(point_filename+'.npy')
    weights = np.load(weights_filename+'.npy')
    ...
    pca_knn = 0
    mean_weights = np.mean(weights, 0)[0]
    ...
    return Shape(pts=pts, features = features, pca_knn = pca_knn, kdtree=kdtree, normals=normals, curv=curvatures, pidx=patch_indices)
...
class Shape():
    def __init__(self, pts, features, pca_knn, kdtree, normals=None, curv=None, pidx=None):
        self.pts = pts
        self.features = features
        self.pca_knn = pca_knn
        self.kdtree = kdtree
...
class PointcloudPatchDataset(data.Dataset):
    ...
    def __init__(self, root, shape_list_filename, patch_radius, points_per_patch, patch_features,
        ...
        for shape_ind, shape_name in enumerate(self.shape_names):
            ...
            weights_filename = os.path.join(self.root, shape_name+'.weights')
            weights = np.loadtxt( weights_filename).astype('float32')
            np.save( weights_filename+'.npy', weights)

            features_filename = os.path.join(self.root, shape_name+'.features')
            features = np.loadtxt( features_filename).astype('float32')
            np.save( features_filename+'.npy', features)
    ...
    def __getitem__(self, index):
        ...
        patch_pfs = torch.zeros(self.points_per_patch*len(self.patch_radius_absolute[shape_ind]), 3, dtype=torch.float)
        pca_pts = torch.zeros(shape.pca_knn, 3, dtype=torch.float)
        patch_normal_pca_sel = torch.zeros(100, 3, dtype=torch.float)
        ...
        for s, rad in enumerate(self.patch_radius_absolute[shape_ind]):
            ...
            point_distances_pca, pca_point_inds = shape.kdtree.query(shape.pts[center_point_ind, :], k=shape.pca_knn)
            rad_pca = max(point_distances_pca)
            pca_pts = torch.from_numpy(shape.pts[pca_point_inds, :])
        ...
        ###  Affnite matrix
        if shape.pca_knn == 32:
            patch_pfs = torch.from_numpy(shape.features[patch_point_inds, 3:6])
        elif  shape.pca_knn == 128:
            patch_pfs = torch.from_numpy(shape.features[patch_point_inds, 9:12])
        elif  shape.pca_knn ==256:
            patch_pfs = torch.from_numpy(shape.features[patch_point_inds, 12:15])
        else:
            patch_pfs = torch.from_numpy(shape.features[patch_point_inds, 15:18])
        A_matrix = torch.exp(-torch.mm(patch_pfs, patch_pfs.transpose(1, 0))**2/0.25)

        ### random a pca normal
        patch_normal_pca = torch.zeros(100, 3, dtype=torch.float)
        pca_pts_center_point = pca_pts - torch.from_numpy(shape.pts[center_point_ind, :])
        pca_pts_center_point = pca_pts_center_point / rad_pca
        
        for i in range(100):
            #point_num = np.random.random_integers(3, shape.pca_knn-1, 1)[0]
            point_num = 4
            id_pca = self.rng.choice(shape.pca_knn, point_num)
            point_pca = pca_pts[id_pca, :]
            point_pca_mean = point_pca.mean(0)
            point_pca = point_pca - point_pca_mean
            trans, _, _ = torch.svd(torch.t(point_pca))
            patch_normal_pca[i,:] = trans[:,2]

        sigm = 0.01
        select_num = -91
        if shape.pca_knn == 450:
            select_num = -101
        if shape.pca_knn == 256:
            select_num = -101
        ...
        if select_num == -91:
            patch_normal_pca_sel[0:90] = patch_normal_pca[id[select_num:-1],:]
        else:
        ...
        if self.use_pca:
            ...
            patch_normal_pca_sel = torch.matmul(patch_normal_pca_sel , trans)
            patch_normal_pca = torch.matmul(patch_normal_pca, trans)
        ...
        for pfeat in self.patch_features:
            if pfeat == 'normal':
                patch_feats = patch_feats + (patch_normal_pca_sel,)
        ...
        return (patch_pts,) + patch_feats + (patch_normal,) + (neighbor_normals,) + (A_matrix,) + (trans,)  + (patch_scale,) #+ (effective_points_num,)
    ...
    def load_shape_by_index(self, shape_ind):
        ...
        weights_filename = os.path.join(self.root, self.shape_names[shape_ind]+'.weights')
        features_filename = os.path.join(self.root, self.shape_names[shape_ind]+'.features')
        ...
        return load_shape(point_filename, weights_filename, features_filename, normals_filename, curv_filename, pidx_filename)
