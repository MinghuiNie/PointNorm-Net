...
def load_shape(point_filename, normals_filename, curv_filename, pidx_filename):
    ...
    return Shape(pts=pts, pca_knn = pca_knn, kdtree=kdtree, normals=normals, curv=curvatures, pidx=patch_indices)
...
class Shape():
    def __init__(self, pts, pca_knn, kdtree, normals=None, curv=None, pidx=None):
        self.pts = pts
...
class PointcloudPatchDataset(data.Dataset):
    ...
    def __init__(self, root, shape_list_filename, patch_radius, points_per_patch, patch_features,
        ...
        for pfeat in self.patch_features:
            if pfeat == 'normal':
                self.include_normals = False####################True
    ...
    def __getitem__(self, index):
        ...
        patch_normal_pca_sel = patch_normal_pca[id[-21:-1],:]
        '''
        if self.use_pca:
        ...
        for pfeat in self.patch_features:
            if pfeat == 'normal':
                patch_feats = patch_feats + (1,)#(patch_normal,)
        ...
        #return (patch_pts,) + patch_feats + (patch_normal,) + (neighbor_normals,) + (trans,)  + (patch_scale,) #+ (effective_points_num,)
        return (patch_pts,) + patch_feats + (trans,)  + (patch_scale,) 
