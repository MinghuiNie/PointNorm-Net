# test_n_est.py run a pretrained MSUNE-Net model and export the normal estimation output
...
from dataset_test import PointcloudPatchDataset, SequentialShapeRandomPointcloudPatchSampler, RandomPointcloudPatchSampler, SequentialPointcloudPatchSampler
...
def parse_arguments():
    ...
    parser.add_argument('--models', type=str, default='DeepFit_jet_order_1_100num_90_100_4point_difpcanum_32_128_256_450_0000005_0.25loss', help='names of trained models, can evaluate multiple models')
    parser.add_argument('--modelpostfix', type=str, default='_model_770.pth', help='model file postfix')
...
def test_n_est(opt):
    ...
    for model_name in opt.models:
       # fetch the model from the log dir
        ...
        output_dir = os.path.join(opt.logdir, model_name, 'results_770')
        ...
        for batchind, data in batch_enum:
            ...
            with torch.no_grad():
                if trainopt.arch == 'simple' or trainopt.arch == 'res' or trainopt.arch == '3dmfv':
                    ...
                    n_est, pred, beta_pred, weights, trans, trans2, neighbor_normals = regressor(points)
                    end_time = time.time()
