...
class PointCloudDenoising(pl.LightningModule):
    ...
    def train_dataloader(self):
        ...
        t = transforms.Compose([
            RandomRotate(30, attr=['pos']),  # rotate normal vectors as well.
            scaler,
        ])
        if self.hparams.dataset.find(';') >= 0:
            print('[INFO] Using multiple datasets for training.')
            dataset = MultipleH5Dataset(self.hparams.dataset.split(';'), 'train', guess_name='value', normal_name=None, batch_size=self.hparams.batch_size, transform=t, random_get=True, subset_size=self.hparams.subset_size)
        else:
            dataset = H5Dataset(self.hparams.dataset, 'train', guess_name='value', normal_name=None, batch_size=self.hparams.batch_size, transform=t)
        return DataLoader(
            dataset, 
    ...
    def val_dataloader(self):
        ...
        if self.hparams.valdata.find(';') >= 0:
            dataset_path = self.hparams.valdata.split(';')[0]
            print('[INFO] Validation dataset %s' % dataset_path)
        else:
            dataset_path = self.hparams.valdata
        return DataLoader(
    ...
    def training_step(self, batch, batch_idx):
        denoised, score = self.forward(batch['pos'])
        guess = batch['vaz']
        #score = score
        #noiseless = batch['clean']
        #normals = batch['normal']
        #input_m = batch['pos'].cpu().detach().numpy()
        #denoised_m = denoised.cpu().detach().numpy()
        #ax = plt.subplot(projection = '3d')
        #ax.scatter(denoised_m[0,:,0], denoised_m[0,:,1], denoised_m[0,:,2],c = 'r')
        #ax.scatter(input_m[0,:,0], input_m[0,:,1], input_m[0,:,2],c = 'b')
        #plt.show()

        loss = self.model.get_loss( preds=denoised, inputs=batch['pos'], guess = guess, score = score)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for key in self.val_noisy_item_keys:
            denoised, score = self.forward(batch[key])
            noiseless = batch['clean']
    ...
    def add_model_specific_args(parent_parser):
        ...
        #parser.add_argument('--valdata', default='./data/10kh5py.h5;./data/20kh5py.h5;./data/30kh5py.h5;./data/40kh5py.h5;./data/50kh5py.h5', type=str)
        parser.add_argument('--valdata', default='./data/patches_20k_1024.h5;./data/patches_10k_1024.h5;./data/patches_30k_1024.h5;./data/patches_50k_1024.h5;./data/patches_80k_1024.h5', type=str)
        parser.add_argument('--dataset', default='./data/10kh5py.h5;./data/20kh5py.h5;./data/30kh5py.h5;./data/40kh5py.h5;./data/50kh5py.h5', type=str)
        ...
        # Loss
        parser.add_argument('--loss_rec', default='unsupervised', type=str, help='Reconstruction loss.')
