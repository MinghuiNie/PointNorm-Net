...
def auto_denoise(args):
    pc_filename = []
    with open(args.input, 'r') as DSFile:        
         for line in DSFile:
            modelName = line.rstrip()
            pc_filename.append(modelName)
    for current_pc_file in pc_filename:
        print('[INFO] Loading: %s' % current_pc_file)
        pc = np.loadtxt(os.path.join('./1our-data-50k', current_pc_file)).astype(np.float32)
        if not os.path.exists(os.path.dirname(os.path.join(args.output, current_pc_file))):
            os.makedirs(os.path.dirname(os.path.join(args.output, current_pc_file)))
        ...
        np.savetxt(os.path.join(args.output, current_pc_file), denoised)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input', type=str, default='./data/our-data-50k/our-025.TXT')
    parser.add_argument('--output', type=str, default='./mhn_predict_12loss_sel_69epoch')
    parser.add_argument('--ckpt', type=str, default='./lightning_logs/version_247/epoch=69.ckpt')
